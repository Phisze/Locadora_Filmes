...
int add_vetorFornecedor(vetorFornecedor* initV, Fornecedor* c) {
    
    if (initV->tamanho == initV->total) {
        initV->itens = realloc(initV->itens, 2 * initV->tamanho * sizeof (Fornecedor));
...
>>>>>>> Stashed changes
    if (codigo < initV->total) {
        ...
        if (initV->total < initV->tamanho && initV->tamanho != 1) {
            initV->itens = realloc(initV->itens, (initV->tamanho - initV->total) * sizeof (Fornecedor));
