...
int add_vetorFilme(vetorFilme * initV, Filme* c) {
    
    if (initV->tamanho == initV->total) {
        initV->itens = realloc(initV->itens, 2 * initV->tamanho * sizeof (Filme));
...
int remove_vetorFilme(vetorFilme* initV, int codigo) {
    Filme initC = {-1, "", -1,-1,' ','*'};
    if (codigo < initV->total) {
        ...
        if (initV->total < initV->tamanho && initV->tamanho != 1) {
            initV->itens = realloc(initV->itens, (initV->tamanho - initV->total) * sizeof (Filme));
