...
int add_vetorCliente(vetorCliente * initV,Cliente*c) {
    if (initV->tamanho == initV->total) {
        initV->itens = realloc(initV->itens, 2 * initV->tamanho * sizeof (Cliente));
...
int remove_vetorCliente(vetorCliente * initV,int codigo) {
    Cliente initC={-1,"","","","","",' ',"","",'*'};
    if (codigo < initV->total) {
       ...
       initV->itens[initV->tamanho - 1] = initC;
        ...
        if (initV->total < initV->tamanho && initV->tamanho != 1) {
            initV->itens = realloc(initV->itens, (initV->tamanho - initV->total) * sizeof (Cliente));
